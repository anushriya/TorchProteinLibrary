...
if __name__ == '__main__':
    ...
    coords, chain_names, resnames, resnums, anames, num_atoms = a2c(angles, sequences)

    # Making a mask on CA, C, N atoms
    is0C = torch.eq(anames[:, :, 0], 67).squeeze()
    is1A = torch.eq(anames[:, :, 1], 65).squeeze()
    is20 = torch.eq(anames[:, :, 2], 0).squeeze()
    is0N = torch.eq(anames[:, :, 0], 78).squeeze()
    is10 = torch.eq(anames[:, :, 1], 0).squeeze()
    isCA = is0C * is1A * is20
    isC = is0C * is10
    isN = is0N * is10
